...
public class TreatmentServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        String path = req.getPathInfo(); // /new, /edit, /delete

        try (Connection conn = DBConnection.getConnection()) {

            if ("/new".equals(path)) {
                String sql = "INSERT INTO treatments " +
                        "(name, description, cost, duration_minutes, active_start, active_end, active_days, dentist_id, active) " +
                        "VALUES (?,?,?,?,?,?,?,?, TRUE)";
                ...
                String[] days = req.getParameterValues("active_days");
                ps.setString(7, days != null ? String.join(",", days) : "");
                ...
                String sql = "UPDATE treatments SET " +
                        "name=?, description=?, cost=?, duration_minutes=?, " +
                        "active_start=?, active_end=?, active_days=?, dentist_id=? " +
                        "WHERE treatment_id=?";
                ...
                // Soft delete – just hide the treatment
